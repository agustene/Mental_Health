      ...
      .on("end", async () => {
        try {
          for (const row of data) {
            ...
            // ========== RELASI ==========
            await db.execute(
              `INSERT INTO pertanyaan_gejala 
               (pertanyaan_id, gejala_id, cf_pakar) 
               VALUES (?, ?, ?)`,
...
/* ==========================================================
   PREVIEW DATASET
========================================================== */
router.get("/dataset-preview", authenticate(["admin"]), async (req, res) => {
  try {
    const [rows] = await db.execute(`
      SELECT 
...
/* ==========================================================
   HAPUS DATASET
========================================================== */
router.delete("/delete-dataset", authenticate(["admin"]), async (req, res) => {
  try {
...
/* ==========================================================
   PENGAJUAN RUJUKAN PSIKOLOG (DENGAN CATATAN ADMIN)
========================================================== */
...
router.patch("/rujukan/:id", authenticate(["admin"]), async (req, res) => {
  ...
  try {
    ...
    res.json({ message: "Rujukan berhasil diperbarui" });
  } catch (err) {
    console.error("Error update rujukan:", err);
    res.status(500).json({ message: "Gagal update rujukan" });
  }
});

// =====================
// UPDATE CATATAN ADMIN SAJA (AUTO SAVE)
// =====================
router.patch(
  "/rujukan/:id/catatan",
  authenticate(["admin"]),
  async (req, res) => {
    ...
    try {
      ...
      res.json({ message: "Catatan admin tersimpan" });
    } catch (err) {
